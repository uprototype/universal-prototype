...
//Source: https://www.advancedswift.com/secure-private-data-keychain-swift/#read-data-from-keychain

//keychain entry of generic password type indexed on service and account keys. use them as follows:
// jmap: service : resourceURL, acccount: UUID
// imap: service: resourceURL, account: account name
...
class KeychainInterface {
    ...
    static func readPasswordMaybe(service: String, account: String) -> String? {
        do{
            let passwordData = try readPassword(service: service, account: account)
            return String(data: passwordData, encoding: .utf8) ?? nil
        }catch{
            print("read password failed with \(error)")
            return nil
        }
    }
    ...
    static func save(password: String, service: String, account: String) throws {
        ...
        guard let passwordData = password.data(using: .utf8) else {
            throw KeychainError.unicodeEncodingFailed
        }
